				...
				if(Bike_RegNo!=null)
				{
					...
					userpoj.setStrInsuredState(strInsuredState);
					 ...
					 String g[]=m.ifBykeRegyr(Bike_RegYear,Bike_Claim);
						...
						String royalRtoCode =royalRegNum.substring(0, 2)+"-"+royalRegNum.substring(2, 4);
						System.out.println("royalRtoCode: "+royalRtoCode);
						String rcity= m.Royalcity(royalRtoCode);
					...
					if(Bike_Model.equals(Variant)){
						libbiVarient=Variant;
					}
					else{
						String libbikevarient[] = Variant.split("\\s",2);//b
						libbiVarient = libbikevarient[1];
					...
					try{//b
					model_code =m.MakeModelLibBike(Bike_Model,libbiVarient); //b
